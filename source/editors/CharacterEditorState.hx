...
class CharacterEditorState extends MusicBeatState
{
	...
	var curAnim:Int = 0;
	static var daAnim:String = 'spooky';
	var goToPlayState:Bool = true;
	var camFollow:FlxObject;

	public function new(Anim:String = 'spooky', goToPlayState:Bool = true)
	{
		super();
		daAnim = Anim;
	...
	override function create()
	{
		...
		changeBGbutton = new FlxUIButton(FlxG.width - 360, 25, "", function()
		{
			onPixelBG = !onPixelBG;
			reloadBGs();
		});
		...
		var tipText:FlxText = new FlxText(FlxG.width - 20, FlxG.height - 5, 0, "ESC - Go back to the Game
	...
	function reloadBGs()
	{
		...
		if (onPixelBG)
		{
			...
			changeBGbutton.label.text = "Regular BG";
		}
		else
		{
			...
			changeBGbutton.label.text = "Pixel BG";
		}
	}
	...
	/*var animationInputText:FlxUIInputText;
		function addOffsetsUI() {
			...
			var addButton:FlxUIButton = new FlxUIButton(animationInputText.x + animationInputText.width + 23, animationInputText.y - 2, "Add", function()
			{
			...
			var removeButton:FlxUIButton = new FlxUIButton(animationInputText.x + animationInputText.width + 23, animationInputText.y + 20, "Remove", function()
			{
				var theText:String = animationInputText.text;
				if(theText != '') {
					for (i in 0...animList.length) {
			...
			var saveButton:FlxUIButton = new FlxUIButton(animationInputText.x, animationInputText.y + 35, "Save Offsets", function()
			{
				saveOffsets();
			});
	...
	var charDropDown:FunkinDropDownMenu;
	...
	function addSettingsUI()
	{
		...
		charDropDown = new FunkinDropDownMenu(10, 30, FunkinDropDownMenu.makeStrIdLabelArray([''], true), function(character:String)
		{
		...
		var reloadCharacter:FlxUIButton = new FlxUIButton(140, 30, "Reload Char", function()
		{
			loadChar(!check_player.checked);
			reloadCharacterDropDown();
		});
	...
	var missDurationStepper:FlxUINumericStepper;
	...
	function addCharacterUI()
	{
		...
		noAntialiasingCheckBox.callback = function()
		{
			...
			if (!noAntialiasingCheckBox.checked && !FunkySettings.noAntialiasing)
			{
		...
		tab_group.add(new FlxText(15, missDurationStepper.y - 18, 0, 'Miss Animation Length:'));
		tab_group.add(new FlxText(positionXStepper.x, scaleStepper.y - 18, 0, 'Scale:'));
		...
		tab_group.add(missDurationStepper);
	...
	var ghostDropDown:FunkinDropDownMenu;
	var animationDropDown:FunkinDropDownMenu;
	var animationInputText:FlxUIInputText;
	...
	function addAnimationsUI()
	{
		...
		animationDropDown = new FunkinDropDownMenu(15, animationInputText.y - 55, FunkinDropDownMenu.makeStrIdLabelArray([''], true),
		...
		ghostDropDown = new FunkinDropDownMenu(animationDropDown.x + 150, animationDropDown.y, FunkinDropDownMenu.makeStrIdLabelArray([''], true),
			function(pressed:String)
			{
				var selectedAnimation:Int = Std.parseInt(pressed);
		...
		var addUpdateButton:FlxUIButton = new FlxUIButton(70, animationIndicesInputText.y + 30, "Add/Update", function()
		{
		...
		var removeButton:FlxUIButton = new FlxUIButton(180, animationIndicesInputText.y + 30, "Remove", function()
		{
			for (anim in char.animationsArray)
			{
				if (animationInputText.text == anim.anim)
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (sender == missDurationStepper)
			{
				char.missDuration = missDurationStepper.value;
			}
	...
	function reloadCharacterImage()
	{
		...
		if (Paths.exists('images/' + char.imageFile + '.txt'))
		{
	...
	{
		var jsonFilter:FileFilter = new FileFilter('Character File', 'json');
		__file = new FileReference();
		__file.addEventListener(Event.SELECT, onLoadComplete);
		__file.addEventListener(Event.CANCEL, onLoadCancel);
		__file.addEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		__file.browse([jsonFilter]);
	}

	function onLoadComplete(_):Void
	{
		__file.removeEventListener(Event.SELECT, onLoadComplete);
		__file.removeEventListener(Event.CANCEL, onLoadCancel);
		__file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);

		try 
		{
			var fullPath:CharacterFile = null;
			var jsonLoaded = cast Json.parse(Json.stringify(__file));
	...
	function reloadAnimationDropDown()
	{
		...
		if (anims.length < 1)
			anims.push(''); // Prevents crash

		animationDropDown.setData(FunkinDropDownMenu.makeStrIdLabelArray(anims, true));
		ghostDropDown.setData(FunkinDropDownMenu.makeStrIdLabelArray(ghostAnims, true));
		reloadGhost();
	}
	...
	function reloadCharacterDropDown()
	{
		...
		charDropDown.setData(FunkinDropDownMenu.makeStrIdLabelArray(characterList, true));
		charDropDown.selectedLabel = daAnim;
	}
	...
	{
		var json:CharacterFile = cast {
			"animations": char.animationsArray,
			"image": char.imageFile,
			"scale": char.jsonScale,
			"sing_duration": char.singDuration,
			"miss_duration": char.missDuration,
