...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	static var TIP_TEXT_MAIN:String = 'JKLI - Move camera (Hold Shift to move 4x faster)
	\nQ/E - Zoom out/in
	\nR - Reset Camera
	\nH - Toggle Speech Bubble
	\nSpace - Reset text';

	static var TIP_TEXT_OFFSET:String = 'JKLI - Move camera (Hold Shift to move 4x faster)
	\nQ/E - Zoom out/in
	\nR - Reset Camera
	\nH - Toggle Ghosts
	...
	override function create()
	{
		...
		box.antialiasing = !FunkySettings.noAntialiasing;
	...
	function addAnimationsUI()
	{
		...
		animationDropDown = new FunkinDropDownMenu(10, 30, FunkinDropDownMenu.makeStrIdLabelArray([''], true), function(animation:String)
		{
	...
	function reloadAnimationsDropDown()
	{
		...
		if (animationArray.length < 1)
			animationArray = [''];
		animationDropDown.setData(FunkinDropDownMenu.makeStrIdLabelArray(animationArray, true));
	}
	...
	function onLoadComplete(_):Void
	{
		...
		var jsonLoaded = cast Json.parse(Json.stringify(_file)); // Exploit(???) for accessing a  variable
