...
enum abstract Action(String) to String from String
{
	...
	var PAUSE = "pause";
	var RESET = "reset";
	var FREEPLAY_RESET = "freeplay-reset";
	var FREEPLAY_LISTEN = "freeplay-listen";
...
class Controls extends FlxActionSet
{
	...
	var _freeplay_reset = new FlxActionDigital(Action.FREEPLAY_RESET);
	var _freeplay_listen = new FlxActionDigital(Action.FREEPLAY_LISTEN);
	var _freeplay_menu = new FlxActionDigital(Action.FREEPLAY_MENU);
	...
	public var FREEPLAY_RESET(get, never):Bool;

	inline function get_FREEPLAY_RESET()
		return _freeplay_reset.check();

	public var FREEPLAY_LISTEN(get, never):Bool;

	inline function get_FREEPLAY_LISTEN()
		return _freeplay_listen.check();

	public var FREEPLAY_MENU(get, never):Bool;

	inline function get_FREEPLAY_MENU()
		return _freeplay_menu.check();
	...
	function getActionFromControl(control:Control):FlxActionDigital
	{
		return switch (control)
		{
			...
			case PAUSE: _pause;
			case RESET: _reset;
			case FREEPLAY_RESET: _freeplay_reset;
			case FREEPLAY_LISTEN: _freeplay_listen;
			case FREEPLAY_MENU: _freeplay_menu;
		}
	}
	...
	function forEachBound(control:Control, func:FlxActionDigital->FlxInputState->Void)
	{
		switch (control)
		{
			...
			case FREEPLAY_RESET:
				func(_freeplay_reset, JUST_PRESSED);
			case FREEPLAY_LISTEN:
				func(_freeplay_listen, JUST_PRESSED);
			case FREEPLAY_MENU:
				func(_freeplay_menu, JUST_PRESSED);
		}
	}
	...
	public function loadKeybinds()
	{
		...
		inline bindKeys(Control.NOTE_LEFT, FunkySettings.controls.get('NOTE_LEFT'));
		inline bindKeys(Control.NOTE_DOWN, FunkySettings.controls.get('NOTE_DOWN'));
		inline bindKeys(Control.NOTE_UP, FunkySettings.controls.get('NOTE_UP'));
		inline bindKeys(Control.NOTE_RIGHT, FunkySettings.controls.get('NOTE_RIGHT'));
		inline bindKeys(Control.UI_LEFT, FunkySettings.controls.get('UI_LEFT'));
		inline bindKeys(Control.UI_DOWN, FunkySettings.controls.get('UI_DOWN'));
		inline bindKeys(Control.UI_UP, FunkySettings.controls.get('UI_UP'));
		inline bindKeys(Control.UI_RIGHT, FunkySettings.controls.get('UI_RIGHT'));
		inline bindKeys(Control.ACCEPT, FunkySettings.controls.get('ACCEPT'));
		inline bindKeys(Control.BACK, FunkySettings.controls.get('BACK'));
		inline bindKeys(Control.PAUSE, FunkySettings.controls.get('PAUSE'));
		inline bindKeys(Control.RESET, FunkySettings.controls.get('RESET'));
		inline bindKeys(Control.FREEPLAY_RESET, FunkySettings.controls.get('FREEPLAY_RESET'));
		inline bindKeys(Control.FREEPLAY_LISTEN, FunkySettings.controls.get('FREEPLAY_LISTEN'));
		inline bindKeys(Control.FREEPLAY_MENU, FunkySettings.controls.get('FREEPLAY_MENU'));
	}
